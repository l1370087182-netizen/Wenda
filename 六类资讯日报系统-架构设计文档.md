# 六类资讯日报系统 · 架构设计文档（v1.1）

> 定稿日期：2026-09-04　|　更新：2026-09-08（多 Agent 架构做实 + 管理后台）　|　部署目标：2C2G 单机　|　定位：定时自动化 + 多 Agent + RAG 的求职展示项目
...
- **管理员**：`adminljj`，专属管理后台——系统概览（用户/文章/日报/发送/轨迹计数 + 最近任务）、用户管理（改角色/验证/通知、删除，含防锁死保护）、任务运维（重跑采集/补发日报）、发送日志、**Agent 轨迹可视化**（每次运行的逐步决策/工具调用）
...
| 调度 | APScheduler（AsyncIOScheduler） | 01:00 采集、08:00 发送两个 Job |
| 多 Agent 编排 | LangGraph（StateGraph） | 确定性 Supervisor（`Command` 相位机路由）+ Worker Agent（ReAct 工具循环），Redis 检查点断点续跑，全程写 `agent_traces` |
| 关系库 | PostgreSQL | 用户/文章/邮件/会话/轨迹等 12 张表 |
...
| 前端 | Vue 3 + Vite + ECharts（frontend/ 独立工程，全手写设计系统，深/浅双主题，无组件库） | 前后端分离；生产为独立 nginx 容器托管静态文件并反代 /api、/mcp，开发期 Vite 代理免 CORS |
| 部署 | docker-compose | frontend(nginx) + app + postgres + redis，单机；app 仅内网可达 |
...
flowchart TB
    ...
    subgraph A["② FastAPI 应用层（单进程）"]
        ...
        Adm["Admin API：用户管理"]
        Sch["APScheduler：01:00 采集 Job · 08:00 发送 Job"]
    end
...
```mermaid
flowchart LR
    Start["入口：01:00 定时任务 / 管理员重跑"] --> Sup["Supervisor 编排 Agent<br/>（时间预算 · 动态重试决策）"]
    Sup --> C1["采集 Agent · 科技"]
    Sup --> C2["采集 Agent · 地缘"]
    Sup --> C3["采集 Agent · 财经"]
...
| Agent | 职责 | 工具 | 模型 |
|---|---|---|---|
| Supervisor | 确定性相位机（`Command` 路由 collect→ingest→analyse→digest→insight→review→revise→finalize）、时间预算/质量决策、批次汇聚写 job_runs | — | —（零 LLM） |
| 采集 Agent ×6 | 快路径直通 Top2 主源（零 LLM）；条数不足才升级为 ReAct 循环自主换源补充 | list_sources / fetch_source | fast（仅升级时调用） |
...
| 项 | 设计 |
|---|---|
| State | 日报 `NewsState`：batch_id、target_date、phase（supervisor 相位机游标）、raw_items、category_reports、recollect_tries、total_ingested、review_round、review_issues、errors；问答 `ChatState`：question、history、qtype、skill_name、pending_queries、subquestions、articles、evidence_round、need_more、answer、sources |
...
| 可观测 | 运行中把每步（supervisor 决策 / 采集工具调用 / 终答）写内存 buffer，`node_finalize` 批量落 `agent_traces` 表，管理后台"Agent 轨迹"页可视化；可选 `LANGCHAIN_TRACING_V2` 接 LangSmith |
...
| 时间 | Job | 流程 |
|---|---|---|
| 01:00 | collect_daily | ① Supervisor 分派 6 个采集 Agent 并行抓取（主源失败自主换源）→ ② 去重（url_hash 唯一 + pgvector 相似度）→ ③ 分析 Agent 逐篇结构化分析 → ④ 入库 PG + pgvector 向量化 → ⑤ 生成 6 份 digest 草稿 + 交叉洞察（status=draft）→ ⑥ 主编审查（≤2 轮修订）→ ⑦ 状态写入 job_runs |
...
> 设计意图：拆成两个 Job，8:00 只做"读库 + 发信"（秒级完成），准时且可重试；01:00–08:00 有 7 小时缓冲应对采集/LLM 波动。
...
| agent_traces | id、run_id、run_kind(daily/chat)、agent_name、step、kind(tool/decision/answer)、tool_name、input、output、model_tier、latency_ms、created_at |
...
| favorites | id、user_id、article_id、created_at（(user_id, article_id) 唯一） |
...
| 管理 | GET /api/admin/users | 用户列表（注册/上次使用/登录时间，分页） |
| 管理 | PATCH /api/admin/users/{id} | 改角色/验证/通知（防锁死：不能自降、不能降级最后一名管理员） |
| 管理 | DELETE /api/admin/users/{id} | 删用户（级联清会话/对话/收藏/模型配置，防锁死） |
| 管理 | GET /api/admin/stats | 系统概览计数（用户/文章/日报/发送/轨迹 + 最近任务） |
| 管理 | GET /api/admin/send-logs | 近 N 天发送日志（按状态过滤） |
| 管理 | GET /api/admin/trace-runs | 近 N 天有轨迹的运行列表 |
| 管理 | GET /api/admin/traces?run_id= | 某次运行的全部 Agent 轨迹 |
| 任务 | GET /api/tasks/status | 采集/发送运行状态（读取 job_runs） |
| 任务 | POST /api/tasks/run | 管理员手动重跑当日采集 |
| 任务 | POST /api/tasks/send | 管理员手动补发当日日报 |
...
│   ├── models.py            # SQLAlchemy 模型（12 张表，含 agent_traces / favorites）
│   ├── api/
│   │   ├── auth.py          # 注册/登录/找回/会话
│   │   ├── users.py         # 设置/订阅
│   │   ├── dashboard.py     # 六类 Top5 / 文章详情
│   │   ├── chat.py          # 多轮研究问答
│   │   └── admin.py         # 管理后台：概览/用户管理/发送日志/Agent 轨迹（require_admin）
│   ├── core/
│   │   ├── security.py      # 密码哈希 / 会话 / Cookie
│   │   ├── deps.py          # 鉴权依赖 + 活跃度更新中间件
│   │   └── email.py         # SMTP 发送 + 验证码生成/校验
│   ├── services/
│   │   ├── collectors/      # base（抓取/RSS/清洗）+ github（trending）
│   │   ├── sources.py       # 境内可达源注册表（六类，快路径 Top2 + 升级换源）
│   │   ├── graph.py         # LangGraph：日报 supervisor 相位机 + 问答子图
│   │   ├── skills.py        # Skill 注册器（扫描 + 渐进式披露）
│   │   ├── vector.py        # pgvector：去重 + 检索
│   │   ├── digest.py        # 邮件内容渲染
│   │   └── scheduler.py     # 01:00 / 08:00 两个 Job
│   ├── skills/              # Agent 技能包（Anthropic Agent Skills 规范）
│   │   ├── weekly-report/SKILL.md
│   │   ├── event-timeline/SKILL.md
│   │   └── impact-analysis/SKILL.md
│   └── static/              # （已废弃：前端迁移至 frontend/）
├── frontend/                # Vue 3 + Vite 前端工程（全手写设计系统，深/浅双主题）
│   ├── src/views/           # 认证 / 看板 / 问答 / 设置 / 管理后台
│   ├── src/styles/          # 设计令牌 + 共享组件样式
│   ├── Dockerfile           # node 构建 → nginx 托管（独立镜像）
│   ├── nginx.conf           # 静态托管 + /api、/mcp 反代 + SPA 回退
│   └── vite.config.js       # 开发代理 /api → FastAPI
├── .env                     # 环境变量（gitignore，模板清单见 README）
├── docker-compose.yml       # frontend(nginx) + app(1g) + postgres(256m) + redis(64m)
├── requirements.txt
└── README.md
```
...
2. **01:00 / 08:00 双 Job 拆分**：凌晨采集，留足 7 小时缓冲，8:00 只读库发信，准时且有重试余地。
...
6. **2C2G 约束**：LLM 全走 API 不本地跑模型；抓取用异步并发；定时任务错峰；前端为独立 nginx 容器（Node 仅构建期需要，运行时零开销），反代 /api 保持同源 Cookie 免跨域问题。
